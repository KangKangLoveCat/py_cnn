    ...
    def forward(self):
        if self.mode == self.TRAIN:
            if self.train_cur_index + self.num >= self.train_img_num:
                ...
                output_index = np.append(t1, t2)
                self.train_epoch = self.train_epoch + 1
                self.train_cur_index = self.train_cur_index + self.num - self.train_img_num
            else:
                output_index = np.arange(self.train_cur_index, self.train_cur_index + self.num)
                self.train_cur_index = self.train_cur_index + self.num
            self.output_images = self.train_images[output_index].reshape(self.num, 1, self.output_h, self.output_w)
            ...
            if self.test_cur_index + self.num >= self.test_img_num:
                ...
                output_index = np.arange(self.test_cur_index, self.test_cur_index + self.num)
                self.test_cur_index = self.test_cur_index + self.num
            self.output_images = self.test_images[output_index].reshape(self.num, 1, self.output_h, self.output_w)
